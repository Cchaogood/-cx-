...
int rel;
char errormsg[50][50];
int fx;                     /* record the index of read fscanf */
int maxfx;
int readtimes;              /* the times of read */
int preread[stacksize];     /* record the pre read */

//int stotableindex[stacksize];	/* 表示执行sto操作时，更新table的index，sti */
//int sti0;
//int sti1;
...
void build(char* fname)
{
    ...
    if ((fin = fopen(fname, "r")) == NULL)      //CX CODE
    {
    ...
    if ((finput = fopen("finput.txt", "r")) == NULL)  /* read文件的读入 */
    {
    ...
    int a;
    while(fscanf(finput,"%d",&a) != EOF) {
        preread[maxfx] = a;
        maxfx++;            /* total number of read */
        printf("maxfx is : %d\n",maxfx);
    }
...
void init()
{
    ...
    strcpy(errormsg[25], "too much read");
    strcpy(errormsg[26], "if 缺少右括号");
}
...
void getch()
{
    if (cc == ll) /* 判断缓冲区中是否有字符，若无字符，则读入下一行字符到缓冲区中 */
    {
        ...
        if (strstr(line, "==") || strstr(line, "!=") || strstr(line, "<") || strstr(line, "<=") || strstr(line, ">") || strstr(line, ">=")|| strstr(line, "odd"))
        {
            rel = true;
        }
    }
...
void interpret()
{
    ...
    if ((fcode = fopen("fcode.txt", "r")) == NULL)      /* 中间代码的读入  */
    {
    ...
    while (fscanf(fcode,"%d%s%d%d", &no, &type, &code[n].l, &code[n].a) != EOF)
    {
    ...
    do {
        ...
        switch (i.f)
        {
            ...
            case opr:	/* 数学、逻辑运算 */
                switch (i.a)
                {
                    ...
                    case 16:/* 读入一个输入置于栈顶 */
                        t = t + 1;
                        s[t] = preread[fx++];
                        printf("read success, read val: %d\n",s[t]);
                        break;
            ...
            case sto:	/* 栈顶的值存到相对当前过程的数据基地址为a的内存 */
                s[base(i.l, s, b) + i.a] = s[t];        //gen(sto, lev-table[i].level, table[i].adr);
                t = t - 1;
                break;
    ...
    do {
        ...
        switch (i.f)
        {
            ...
            case lod:	/* 取相对当前过程的数据基地址为a的内存的值到栈顶 */
                ...
                break;
            case sto:	/* 栈顶的值存到相对当前过程的数据基地址为a的内存 */
                s[i.a] = s[t];    //i.f l a   gen(sto, lev-table[i].level, table[i].adr);	/* 将栈顶内容送入变量单元中 */
                t = t - 1;
                break;
            case ini:	/* 在数据栈中为被调用的过程开辟a个单元的数据区 */
                t = t + i.a;
